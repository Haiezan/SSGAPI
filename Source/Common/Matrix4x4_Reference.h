...
class _SSG_DLLIMPEXP Matrix4x4CPU
{
public:
	union
	{
...
public:
	...
	inline void SetColumn(int column, const Vector4CPU &vec)
	{
	...
	// result.w is garbage
	inline Vector4CPU TransformVec3(const Vector4CPU &vec)
	{
		Vector4CPU result = vec * (*(const Vector4CPU*)this);
		return result;
	}

	void Rotate( const Vector4CPU &v, float radian)
	{
		float x = v.x;
		float y = v.y;
		float z = v.z;
	...
	void Rotate_Replace( const Vector4CPU &v, float radian)
	{
		float x = v.x;
		float y = v.y;
		float z = v.z;
...
//	Vector4CPU result;
//
//	result[0] = Vector4Dot(vec, matrix[0]).GetX();
//	result[1] = Vector4Dot(vec, matrix[1]).GetX();
//	result[2] = Vector4Dot(vec, matrix[2]).GetX();
//	result[3] = Vector4Dot(vec, matrix[3]).GetX();
//
//	return result;
//}
