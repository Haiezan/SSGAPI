...
const float Sys_FileVersion=2018.0f; //IO文件版本号，递增精度>0.001，使用该数据时应注意float的精度问题，应留有余量例如设置误差0.05f。
...
const int Sys_SubSectionParaNum=12; //子截面几何参数个数，要与实际情况精确相符
...
const int Sys_MaxSteelNum =8;  //FEA计算最大钢材数
const int Sys_Conc_Para_Inter_Num =7; //计算接口中混凝土参数个数，排在Sys_Conc_Para_Num前面
...
const int Sys_MaxModeNum =100 ; //使用振型阻尼时的最大振型数
...
const int Sys_CheckErrors =16; //检查模型的项数
...
const int Sys_ConcFibreLen=3328;  //最大混凝土纤维长度
const int Sys_EdgeSec =256;        //最大约束边缘构造柱截面信息长度
const int Sys_LinkSec =900;        //最大阻尼器截面信息长度	//乔保娟 2015.5.20		=30*num	最多30个截面
const int Sys_Geo =1500 ;           //最大截面几何信息长度,截面数=1500/6=250	//乔保娟 2015.9.10
const int Sys_Geo_Num =6;          //材料参数个数
const int Sys_Max_BC =8192  ;      //最大约束结点数
...
enum RUN_CASE
{
	...
	RUN_EXPSTATIC			=0x1000,					//显式竖向荷载加载
	RUN_EXPDYNA			=0x2000,					//显式动力时程分析
	RUN_CHECKMODEL		=0x10000,					//仅检查模型
	RUN_EXPSTATIC_EXPDYNA	=RUN_EXPSTATIC|RUN_EXPDYNA,	//显式竖向荷载加载+显式动力时程分析
